...
dtcreate(dt_impl_t impl){
    struct _dict* dict = jcalloc(sizeof(struct _dict));
    if (!dict) return 0;
    dict->impl = impl;
    dict->size = DICT_DEFAULT_SIZE;
    dict->used = 0;
    dict->table = jcalloc(dict->size*(sizeof(struct _dict_entity*)));
...
dtset(dict_t dict, void* key, void* val){
    ...
    if (e){
        ...
        if (e){
            ...
            if (!(e = jmalloc(sizeof(struct _dict_entity))))return -1;
            pe->next = e;
            e->next = 0;
        }
    } else{
        e = jmalloc(sizeof(struct _dict_entity));
        if (!e) return -1;
        e->next = 0;
        dict->table[index] = e;
    }
...
dtsetraw(dict_t dict, void* key){
    ...
    int hash = dt_hash_key(dict, key);
    int index = hash& (dict->size - 1);
    dt_entity_t entity = jmalloc(sizeof(struct _dict_entity));
    entity->next = 0;
    if (!entity) return 0;
    dt_entity_t e = dict->table[index];
    if (e){
...
dtrehash(dict_t dict){
    ...
    dt_entity_t* table = jmalloc( size*sizeof(dt_entity_t));
    if (!table) {
        dtfree(dict);
        return 0;
    }
...
dtiterator(dict_t dict){
    dt_iterator_t itr = jmalloc(sizeof(struct _dict_iterator));
    if (!itr)return 0;
    itr->d = dict;
    itr->i = 0;
    itr->e = 0;
